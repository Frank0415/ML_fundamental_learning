...
class MultiHeadAttention(Module):
    ...
    def project_to_query_key_value(self, x):
        ...
        q = self.q_projection(x2d).view(
            batch_size, seq_len, self.n_head, self.attn_hidden_dim
        )
        kT = self.k_projection(x2d).view(
            batch_size, seq_len, self.n_head, self.attn_hidden_dim
        )
        v = self.v_projection(x2d).view(
            batch_size, seq_len, self.n_head, self.attn_hidden_dim
        )
        q = q.permute(0,2,1,3)
...
class TransformerLayer(Module):
    ...
    def forward(self, x):
        ...
        if not self.use_fused_kernel:
            x = x + self.attention(
                self.ln_1(x.view(batch_size * seq_len, x_dim)).view(
                    batch_size, seq_len, x_dim
                )
            )
            x = x + self.ff(
                self.ln_2(x.view(batch_size * seq_len, x_dim)).view(batch_size, seq_len, x_dim)
            )
        else:
            # BEGIN ASSIGN3_3
            raise NotImplementedError
