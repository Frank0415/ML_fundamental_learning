...
- **Dev host**：macOS Apple M5 (Metal 4)，无 NVIDIA GPU，不支持 CUDA。所有真实 reference inference 需要远程 CUDA GPU。
...
- **T14 Reference Image**：脚手架就绪，但三模型（Sana/SD3/FLUX）均因环境依赖缺失而阻塞。远程 CUDA GPU 不可用。
- **T15 Reference Video**：脚手架就绪，3 个 blocker 占位文件表示"脚本已就绪但尚未在 GPU 上执行"。CogVideoX-2B 无授权障碍，LTX-Video 2B 开放协议。
- **Video 规格**：建议 在中等显存配置下默认使用 ≤16 帧 × 256² × ≤8 步。严格按此执行，不无限调参。
...
> **本页结论**
> 
> 本项目在 6 周内完成了 17/18 个任务，产出了 12 页静态知识库、20 篇学习资料、10 个核心模块（36 个测试）、5 组实验目录、8 份报告。真实 reference inference 因远程 GPU 环境未就绪而阻塞，但脚手架已完整就绪——环境就绪后单条命令即可运行。系统优化实验数据表明：prompt embedding cache（52% hit）、latent buffer（91.8% allocation 节省）、CFG batching（1.01-1.02× 加速）是有效的扩散推理优化；attention memory O(N²) 是真实瓶颈，受限显存配置下高分辨率必须 memory-efficient attention。
