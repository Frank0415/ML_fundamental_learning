...
1. [推理全景](#0-推理全景)
2. [prompt → tokenizer → text encoder → prompt embeddings](#1-prompt--tokenizer--text-encoder--prompt-embeddings)
3. [noise / latent 初始化](#2-noise--latent-初始化)
4. [timestep / sigma schedule](#3-timestep--sigma-schedule)
5. [denoiser / vector field / DiT forward](#4-denoiser--vector-field--dit-forward)
6. [classifier-free guidance (CFG)](#5-classifier-free-guidance-cfg)
7. [scheduler update](#6-scheduler-update)
...
特别强调：
- `scheduler.py` 的输入是经 CFG 合并后的 vector field，不是 raw denoiser 输出。
- `pipeline.py` 的 CFG 逻辑必须在 scheduler.step() 之前执行。
- `dit.py` 的 forward 需要同时接受 latent tokens 和 text tokens（MMDiT joint attention）或分开接受（cross-attention）。
