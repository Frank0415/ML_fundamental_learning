...
// template <typename T>
extern "C" {
void launch_attn_softmax_bw(float *out_grad, const float *soft_outp, int rows,
  ...
  int float_size = sizeof(float);
  int grad_size = float_size * rows * softmax_len;
  int outp_size = float_size * rows * softmax_len;

  float *d_grad, *d_outp;
  cudaMalloc((void **)&d_grad, grad_size);
  cudaMalloc((void **)&d_outp, outp_size);
  cudaMemcpy(d_grad, out_grad, grad_size, cudaMemcpyHostToDevice);
  cudaMemcpy(d_outp, soft_outp, grad_size, cudaMemcpyHostToDevice);
  // Launch kernel
  ...
  // Copy back to the host
  cudaMemcpy(out_grad, d_grad, grad_size, cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
